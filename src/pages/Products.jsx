import { Button, Card, Container, Rating, TextField, Typography } from '@mui/material'
...
const Products = () => {
  ...
  return (
    <>
    <Typography variant="h3" textAlign='center' paddingBottom="5px">Shop Now</Typography>
  ...
  <Typography className='search' ><TextField sx={{width:270}} placeholder='search here...' /></Typography>
        ...
        Product.map((data, index) => {
          return (
            <Card className="product-item" variant="outlined" key={index}>
              ...
              <Typography component="p"><Rating size='small' value={data.rating.rate} precision={0.5} readOnly/>({data.rating.count})</Typography>
              <Button variant='contained' onClick={()=>dispatch(AddCart(data))} color='secondary'>Add to card</Button>
            </Card>
          );
